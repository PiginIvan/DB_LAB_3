...
Некоторые библиотеки предоставляют возможность создания базы данных, однако я использовал `sqlite3 + pandas` для создания .db файла и `sqlalchemy + pandas` для загрузки данных в `PostgreSQL` один раз перед запуском запросов. В каждой библиотеке обращался к уже созданному файлу .db или `PostgreSQL`.
...
`PostgreSQL` использовался для `psycopg2`, `sqlalchemy` и `pandas`. Если таблица уже имеется в `PostgreSQL`, то повторное создание в программе производиться не будет.

## Сравнительные графики и результаты запусков
...
Для начала нужно задать параметры подключения к `PostgreSQL` через такой словарь.
...
Результаты запусков показывают, что `sqlite3` работает быстрее, чем большинство библиотек, однако не максимально эффективно. Есть возможность провести индексацию таблицы, чтобы ускорить выполнение запросов. Занимает 2-ое место по времени после `duckdb`. Это может быть связано с особенностями архитектуры SQLite, на чтение запросов она работает достаточно быстро. Также чтение данные идет из файла, а не через сервер.
...
Также есть возможность писать запросы иначе, используя dataframe, однако для этого требуется загрузка в память. Пример первого запроса приведен ниже. Такой вариант будет работать быстрее, но занимать память, соответственно необходимо выбирать вариант написания кода в зависимости от ситуации.
...
По графикам можно заметить, что `sqlalchemy` показала самый слабый результат среди всех рассмотренных библиотек. Предоставляет возможность подключения как к .db файлу, так и к `PostgreSQL`, я выбрал `PostgreSQL`, что тоже в значительной степени повлияло на результаты. Главным фактором является ORM, что замедляет выполнение всех запросов. 
